        ...
        <c:if test="${sessionScope.user_role=='DOCTOR' or sessionScope.user_role=='PHARMACIST'}">
            <form action="${pageContext.request.contextPath}/pharmacy" method="GET">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="recipe_id" class="form-control" id="id"
                           placeholder="Enter recipe id">
                </div>
                <div class="form-group">
                    <label for="id_patient">Patient's id</label>
                    <input type="number" name="recipe_patient" class="form-control" id="id_patient"
                           placeholder="Enter patient's id">
                </div>
                <div class="form-group">
                    <label for="id_medicine">Medicine id</label>
                    <input type="number" name="recipe_medicine" class="form-control" id="id_medicine"
                           placeholder="Enter medicine id">
                </div>
                <div class="form-group">
                    <label for="id_doctor">Doctor's id</label>
                    <input type="number" name="recipe_doctor" class="form-control" id="id_doctor"
                           placeholder="Enter doctor's id">
                </div>
                <div class="form-group">
                    <label for="dose">Dose</label>
                    <input type="number" step="0.01" name="recipe_dose" class="form-control" id="dose"
                           placeholder="Enter dose">
                </div>
                <div class="form-group">
                    <label for="duration">Duration</label>
                    <input type="number" name="recipe_duration" class="form-control" id="duration"
        ...
        <div class="row">
            ...
            <div class="col-md-2">
                <c:if test="${sessionScope.user_role == 'PATIENT'}">
                    <a href="/pharmacy?command=add_request&request_recipe=${recipe.id}">  <button type="button" class="btn btn-info">Ask for prolongation</button></a>
                </c:if>
                <c:if
                        test="${sessionScope.user_role == 'DOCTOR' && sessionScope.user_status=='ACTIVE'}">
                    <button type="button" onclick="openUpdate()" class="btn btn-warning">Prolong</button>
                </c:if>
            </div>
            <div class="col-md-3 ">
